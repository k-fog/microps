...
tcp_pcb_select(ip_endp_t key1, ip_endp_t key2)
{
    ...
    for (pcb = pcbs; pcb < tailof(pcbs); pcb++) {
        /* local: port must be equal */
        if (pcb->local.port != key1.port) {
            continue;
        }
        /* local: address must be equal, unless either side is a wildcard */
        if (pcb->local.addr != key1.addr &&
            pcb->local.addr != IP_ADDR_ANY &&
            key1.addr != IP_ADDR_ANY) {
            continue;
        }
        /* remote: exact match, or a wildcard on either side */
        ...
        /* keep looking for a connected PCB, fall back to LISTEN */
    ...
    case TCP_STATE_LISTEN:
        ...
        */
        if (TCP_FLG_ISSET(flags, TCP_FLG_SYN)) {
            ...
            // ignore: any other incoming control or data
            return;
        }
